...
test('calculateQuote returns correct estimate for all plans included', () => {
  ...
  const healthPlan: HealthPlan = { id: '1', name: 'Plan ORO', type: 'health', details: 'Asegurado Principal', cost: 0, currency: 'USD', insuredPrimary: 790.25, spouse: 640.00, children: 282.96 };
  ...
  expect(quote.subTotal).toBeCloseTo(1963.46); // Sum of all costs
  expect(quote.tax).toBeCloseTo(294.519); //  15% tax
  expect(quote.totalAmount).toBeCloseTo(2257.979); // SubTotal + Tax
});